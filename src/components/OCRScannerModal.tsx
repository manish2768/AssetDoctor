...
export const OCRScannerModal: React.FC<OCRScannerModalProps> = ({
  ...
  const capturePhoto = async () => {
    ...
    if (ctx) {
      ...
      stopCamera();
      const compressedDataUrl = await compressImage(rawDataUrl, 800, 800, 0.7);
      setImagePreview(compressedDataUrl);
      triggerScan({ base64Image: compressedDataUrl, mimeType: 'image/jpeg' });
    }
  };
  ...
  const processFile = (file: File) => {
    ...
    reader.onload = async () => {
      ...
      const compressedDataUrl = await compressImage(result, 800, 800, 0.7);
      setImagePreview(compressedDataUrl);
      triggerScan({ base64Image: compressedDataUrl, mimeType: file.type || 'image/jpeg' });
    };
