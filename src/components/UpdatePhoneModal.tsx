...
export const UpdatePhoneModal: React.FC<UpdatePhoneModalProps> = ({
  ...
  const isFirstTimePhone =
    !currentPhone ||
    currentPhone.trim() === '' ||
    currentPhone.includes('00000') ||
    currentPhone.toLowerCase().includes('not set');
  ...
  const handleSendOtp = (e: React.FormEvent) => {
    ...
    if (isFirstTimePhone) {
      // First-time save: Skip forced OTP and save directly
      setErrorMsg('');
      setStep('SAVING');
      ...
      // Modifying existing phone: Trigger OTP verification
            ...
            <form onSubmit={handleSendOtp} className="space-y-4">
              {isFirstTimePhone && (
                <div className="p-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs font-semibold flex items-center gap-2">
                  ...
                  <span>First-Time Save: Your phone number will be saved directly to your profile without OTP verification.</span>
                </div>
              )}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1.5 flex items-center gap-1">
                  ...
                  {isFirstTimePhone ? 'Enter Mobile Phone Number:' : 'Enter New Mobile Phone Number:'}
                </label>
                ...
                <p className="text-[11px] text-slate-500 mt-1.5">
                  {isFirstTimePhone
                    ? 'Saving your phone number for the first time directly updates your account profile.'
                    : 'We will send a 6-digit OTP code to verify ownership before updating your existing phone number.'}
                </p>
              </div>
                ...
                >
                  {isFirstTimePhone ? (
                    <>
                      ...
                      <span>Save Phone Number Directly</span>
                    </>
                  ) : (
                    <>
